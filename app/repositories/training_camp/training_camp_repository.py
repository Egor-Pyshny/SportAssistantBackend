...
class TrainingCampRepository:
    ...
    async def get_all_previous_camps(
    ...
    async def get_all_current_camps(
        self, user_id: str, current_date: date
    ) -> List[TrainingCamp]:
        query = select(TrainingCamp).where(
            and_(
    ...
    async def get_all_future_camps(
        self, user_id: str, current_date: date
    ) -> List[TrainingCamp]:
        query = select(TrainingCamp).where(
    ...
    async def get_all_days(self, id: str) -> List[TrainingCampDay]:
        query = (
            select(TrainingCampDay).options(subqueryload(TrainingCampDay.days)).where(TrainingCampDay.id == id)
        )
        result = await self.db.execute(query)
